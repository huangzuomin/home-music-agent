...
## A4 核查结论（2026-09-18 早上）

| 检查项 | 结果 |
|---|---|
| NAS 挂载 | ✅ NFS v3 rw 正常（192.168.50.247:/volume4/music → /mnt/music） |
| /mnt/music 近 24h 写入 | ✅ 有新文件（enrich + 点播补库） |
| 根分区同名空目录风险 | 未发生（/mnt/music 是 NAS 挂载点，不落到根分区） |
| fetcher 近 24h 作业 | done + rejected 正常交替，无 500/异常 |

**A4 结论**：NAS 挂载正常，写入路径正确。X08 风险（根分区同名目录）未发生。
但该风险仍在——如果 NAS 短暂掉线，/mnt/music 变为空目录，写入会静默丢失。
缓解：tools/preflight.py 加入部署前检查；长期方案由 IMP-04 的 readiness 核验兜底。
