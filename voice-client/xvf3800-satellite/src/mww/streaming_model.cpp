...
char g_mww_io_info[160] = {0};
const char *streaming_model_io_info() { return g_mww_io_info; }

bool StreamingModel::load_now() { return load_model_(); }
...
bool StreamingModel::load_model_() {
  ...
  if (this->interpreter_ == nullptr) {
    ...
    {   // 诊断：输入/输出张量类型、维度、量化
      TfLiteTensor *in = this->interpreter_->input(0);
      TfLiteTensor *out = this->interpreter_->output(0);
      snprintf(g_mww_io_info, sizeof(g_mww_io_info),
               "io: in type=%d dims=[%d,%d,%d] scale=%f zero=%d | out type=%d dims=[%d,%d] scale=%f zero=%d",
               in->type, in->dims->data[0], in->dims->data[1], in->dims->data[2],
               (double)in->params.scale, in->params.zero_point,
               out->type, out->dims->data[0], out->dims->data[1],
               (double)out->params.scale, out->params.zero_point);
    }
